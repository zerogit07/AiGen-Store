...
def get_category_keyboard(categories, page, total_pages, offset):
    ...
    for i, (cat_id, _) in enumerate(categories, start=offset + 1):
        buttons.append(
            InlineKeyboardButton(
                text=pad_center(str(i), BUTTON_WIDTH), callback_data=f"cat_{cat_id}"
            )
        )
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    separator = "━" * 17
    text = f"🛍 *DAFTAR KATEGORI • Page {page}/{total_pages}*\n{separator}\n"
    ...
    if banner:
        await message.answer_photo(
            banner, caption=text, parse_mode="Markdown", reply_markup=keyboard
        )
    else:
