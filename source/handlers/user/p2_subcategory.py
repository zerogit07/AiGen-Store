...
from source.database.queries import (
    get_subcategories_by_category, get_category_name, get_stock_for_subcategory,
...
def get_subcategory_keyboard(subcategories, page, total_pages, offset, category_id):
    ...
    nav = []
    if page > 1:
        nav.append(InlineKeyboardButton(text="◀", callback_data=f"sub_page_{category_id}_{page-1}"))
    if page < total_pages:
        nav.append(InlineKeyboardButton(text="▶", callback_data=f"sub_page_{category_id}_{page+1}"))
    if nav:
        rows.append(nav)
...
async def show_subcategories_by_edit(callback: CallbackQuery, state: FSMContext, category_id: int, page: int = 1):
    ...
    await state.update_data(selected_category_id=category_id, subs=subs,selected_category_name=cat_name)
...
async def select_subcategory(callback: CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    subs = data.get('subs', [])
    sub_name = next((name for sid, name, _ in subs if str(sid) == str(sub_id)), "Produk")
