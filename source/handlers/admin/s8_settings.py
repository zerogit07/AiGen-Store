...
async def settings_menu(callback: CallbackQuery):
    ...
    used_count = await get_used_items_count()

    text = (
        f"⚙️ *Pengaturan Toko*\n\n"
        f"🖼️ *Banner Toko:* {'✅ Ada' if banner_id else '❌ Belum diatur'}\n"
        f"💳 *QRIS:* {'✅ Ada' if qris_id else '❌ Belum diatur'}\n"
        f"🗑️ *Auto Delete Item Terpakai:* {auto_days} hari\n"
        f"📦 *Item Terpakai Saat Ini:* {used_count} item\n\n"
        f"Pilih tindakan:"
    )
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        ...
        InlineKeyboardButton(text="🧹 Manual Delete Item Terpakai", callback_data="settings_manual_delete_used")],
...
# ── Manual Delete ──
@router.callback_query(F.data == "settings_manual_delete_used")
async def manual_delete_used(callback: CallbackQuery):
    count = await get_used_items_count()
    if count == 0:
        await callback.answer("Tidak ada item terpakai saat ini.", show_alert=True)
        return

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Yakin, hapus semua", callback_data="confirm_delete_used")],
        [InlineKeyboardButton(text="❌ Batal", callback_data="admin_settings")]
    ])
    await callback.message.answer(
        f"⚠️ Kamu akan menghapus *{count} item terpakai*.\nLanjutkan?",
        parse_mode="Markdown",
        reply_markup=keyboard
    )
    await callback.answer()

@router.callback_query(F.data == "confirm_delete_used")
async def confirm_delete_used(callback: CallbackQuery):
    deleted = await delete_all_used_items()   # ← fungsi baru
    await callback.message.answer(
        f"✅ *Manual Delete Selesai*\n📦 Item terpakai yang dihapus: {deleted} item.",
        parse_mode="Markdown"
    )
