...
async def delete_category(cat_id: int) -> bool:
    """Hapus kategori, lalu bersihkan subkategori dan item tak bertuan yang dihasilkan."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        # 1. Hapus kategori
        await db.execute("DELETE FROM categories WHERE id = ?", (cat_id,))
        # 2. Hapus subkategori yang sekarang tak bertuan
        await db.execute(
            "DELETE FROM subcategories WHERE category_id NOT IN (SELECT id FROM categories)"
        )
        # 3. Hapus item yang sekarang tak bertuan
        await db.execute(
...
async def get_subcategories_by_category(
    category_id: int, limit: int = 10, offset: int = 0
):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        async with db.execute(
            "SELECT id, name, price FROM subcategories WHERE category_id = ? ORDER BY name LIMIT ? OFFSET ?",
            (category_id, limit, offset),
        ) as cursor:
            rows = await cursor.fetchall()
        async with db.execute(
            "SELECT COUNT(*) FROM subcategories WHERE category_id = ?", (category_id,)
        ) as cursor:
            total = (await cursor.fetchone())[0]
        return rows, total
...
async def add_subcategory(category_id: int, name: str, price: int):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        cur = await db.execute(
            "INSERT INTO subcategories (category_id, name, price) VALUES (?, ?, ?)",
...
async def update_subcategory(sub_id: int, new_name: str = None, new_price: int = None):
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        if new_name:
            await db.execute(
                "UPDATE subcategories SET name = ? WHERE id = ?", (new_name, sub_id)
            )
        if new_price is not None:
            await db.execute(
                "UPDATE subcategories SET price = ? WHERE id = ?", (new_price, sub_id)
            )
        await db.commit()


async def delete_subcategory(sub_id: int) -> bool:
    """Hapus subkategori, lalu bersihkan item tak bertuan yang dihasilkan."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        # 1. Hapus subkategori
        await db.execute("DELETE FROM subcategories WHERE id = ?", (sub_id,))
        # 2. Hapus item yang sekarang tak bertuan (subcategory_id tidak ada di tabel subcategories)
        await db.execute(
            "DELETE FROM items WHERE subcategory_id NOT IN (SELECT id FROM subcategories)"
        )
        await db.commit()
...
async def get_items_by_subcategory(
    subcategory_id: int, limit: int = 10, offset: int = 0
):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        async with db.execute(
            "SELECT id, code, is_used FROM items WHERE subcategory_id = ? ORDER BY id LIMIT ? OFFSET ?",
            (subcategory_id, limit, offset),
        ) as cursor:
            rows = await cursor.fetchall()
        async with db.execute(
            "SELECT COUNT(*) FROM items WHERE subcategory_id = ?", (subcategory_id,)
        ) as cursor:
            total = (await cursor.fetchone())[0]
        return rows, total
...
async def add_item(subcategory_id: int, code: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        cur = await db.execute(
            "INSERT INTO items (subcategory_id, code) VALUES (?, ?)",
            (subcategory_id, code),
        )
        await db.commit()
        return cur.lastrowid
...
async def mark_item_used(item_id: int, order_id: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        await db.execute(
            "UPDATE items SET is_used = 1, order_id = ?, used_at = CURRENT_TIMESTAMP WHERE id = ?",
            (order_id, item_id),
        )
        await db.commit()
...
async def delete_used_items(subcategory_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        await db.execute(
            "DELETE FROM items WHERE subcategory_id = ? AND is_used = 1",
            (subcategory_id,),
        )
        await db.commit()
...
async def get_subcategory_price_and_stock(subcategory_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        async with db.execute(
            "SELECT price FROM subcategories WHERE id = ?", (subcategory_id,)
        ) as cursor:
            row = await cursor.fetchone()
...
async def get_available_items_by_subcategory(
    subcategory_id: int, limit: int = 10, offset: int = 0
):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        cursor = await db.execute(
            "SELECT id, code, is_used FROM items WHERE subcategory_id = ? LIMIT ? OFFSET ?",
            (subcategory_id, limit, offset),
        )
...
async def get_item_by_id(item_id: int):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        cursor = await db.execute(
            "SELECT id, subcategory_id, code, is_used FROM items WHERE id = ?",
            (item_id,),
        )
...
async def edit_item_code(item_id: int, new_code: str) -> bool:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        item = await db.execute_fetchall(
            "SELECT subcategory_id FROM items WHERE id = ?", (item_id,)
        )
        if not item:
            return False
        sub_id = item[0][0]
...
async def delete_single_item(item_id: int) -> bool:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        cursor = await db.execute(
            "DELETE FROM items WHERE id = ? AND is_used = 0", (item_id,)
        )
        await db.commit()
...
async def get_subcategory_by_id(sub_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        async with db.execute(
            "SELECT id, category_id, name, price FROM subcategories WHERE id = ?",
            (sub_id,),
        ) as cursor:
            return await cursor.fetchone()
...
async def export_items_csv(subcategory_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        async with db.execute(
            "SELECT code FROM items WHERE subcategory_id = ? AND is_used = 0",
            (subcategory_id,),
        ) as cursor:
            rows = await cursor.fetchall()
...
async def get_subcategory_by_name(category_id: int, name: str):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        async with db.execute(
            "SELECT id, name, price FROM subcategories WHERE category_id = ? AND name = ?",
            (category_id, name),
        ) as cursor:
            return await cursor.fetchone()
...
async def delete_all_used_items() -> int:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        await db.execute(
            "DELETE FROM orders WHERE item_id IN (SELECT id FROM items WHERE is_used = 1)"
        )
        # Baru hapus item-nya
...
async def update_subcategory_details(subcategory_id: int, name: str, price: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("PRAGMA foreign_keys = ON")
        await db.execute(
            "UPDATE subcategories SET name = ?, price = ? WHERE id = ?",
            (name, price, subcategory_id),
        )
        await db.commit()
...
# Modifikasi get_available_items_by_subcategory agar mengembalikan is_used
...
async def add_notification(
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """,
            (type, title, message, page, tab, related_id),
        )
