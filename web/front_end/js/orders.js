...
function renderOrderList(orders, total, page) {
    ...
    if (currentOrdersTab === 'masuk') {
        ...
        let html = '<div class="table-responsive"><table class="order-table"><thead><tr>';
        html += '<th>No</th><th>Order ID</th><th>User</th><th>Produk</th><th>Qty</th><th>Total</th>';
        if (currentOrdersTab === 'riwayat') {
        ...
        orders.forEach((o, index) => {
            ...
            html += `<tr>
                ...
                <td>Rp${o.total}</td>`;
            if (currentOrdersTab === 'riwayat') {
                let statusLabel = o.status === 'approved' ? 'Disetujui' : 'Ditolak';
                let badgeClass = o.status === 'approved' ? 'badge-success' : 'badge-danger';
...
function pagination(total, currentPage) {
    ...
    if (currentPage > 1) html += `<button class="btn-page" onclick="renderOrders(${currentPage-1})">◀</button>`;
    html += `<span>Hal ${currentPage}/${totalPages}</span>`;
    if (currentPage < totalPages) html += `<button class="btn-page" onclick="renderOrders(${currentPage+1})">▶</button>`;
    html += '</div>';
    return html;
}
...
async function rejectOrder(orderId) {
    window.showConfirmModal('Tolak pesanan ini?', async () => {
        const res = await fetch(`/api/orders/reject/${orderId}`, { method: 'POST' });
        const data = await res.json();
        showToast(data.message, data.success ? 'success' : 'error');
        if (data.success) renderOrders(currentOrdersPage);
    });
...
async function bulkAction(action) {
    window.showConfirmModal(`Yakin ${action === 'approve-all' ? 'setujui' : 'tolak'} semua pesanan masuk?`, async () => {
        const res = await fetch(`/api/orders/${action}`, { method: 'POST' });
        const data = await res.json();
        showToast(data.message, data.success ? 'success' : 'error');
...
async function deleteHistory() {
    ...
    window.showConfirmModal(`Hapus riwayat (${currentFilter || 'semua'})?`, async () => {
