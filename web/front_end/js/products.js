...
(function () {
    ...
    async function renderCategories(page) {
        ...
        try {
            const [catRes, sumData] = await Promise.all([
                fetch(
                    `/api/products/categories?page=${page}&limit=${itemsPerPage}`
                ).then(r => r.json()),
                fetchSummary("kategori")
            ]);
            let html = renderSummaryGrid(sumData, "kategori");
            html +=
                '<button class="btn btn-primary btn-full" onclick="window.showCategoryModal()">➕ Tambah Kategori</button>';
            html += '<div id="catList">';
                    ...
                    <div class="list-row">
                        ...
                        <div class="list-row-actions">
                            <button class="btn-icon"
                                onclick='window.editCategory(${c.id}, ${JSON.stringify(c.name)})'>
                            ✏️
                            </button>
            ...
            content.innerHTML =
                '<div class="placeholder"><p>❌ Gagal memuat kategori.</p></div>';
        }
    }
    ...
    window.editCategory = function (id, name) {
        console.log("[EDIT CLICK]", id, name);

        window.showCategoryModal(Number(id), String(name));
    };
    ...
    window.updateCategory = async function (id) {
        ...
        try {
            ...
            if (data.success) {
                renderCategories(currentProductPage);
            }
        } catch (e) {
    ...
    // ═══════════════ SUBKATEGORI ═══════════════
    window.renderSubcategorySelector = async function (
        page = 1,
        selectedCatId = null
    ) {
        currentProductPage = page;
        ...
        <div class="form-group" onclick="window.showSearchableDropdown(document.getElementById('catSelect'))">
            ...
            <div class="form-input product-select"
        ...
        content.innerHTML = html;
        if (selectedCatId)
            await window.renderSubcategories(selectedCatId, page);
    };

    window.renderSubcategories = async function (catId, page) {
        console.log(
            `[DEBUG] Rendering subcategories for catId: ${catId}, page: ${page}`
        );

        const subList = document.getElementById("subList");
        if (subList) {
            subList.innerHTML =
                '<div class="placeholder"><p>⏳ Memuat data...</p></div>';
        }

        try {
            const response = await fetch(
                `/api/products/subcategories?category_id=${catId}&page=${page}&limit=${itemsPerPage}`
            );
            ...
            const sumData = await fetchSummary(
                "subkategori",
                `&category_id=${catId}`
            );
            ...
            if (!subRes.data || subRes.data.length === 0) {
                html +=
                    '<div class="placeholder"><p>📭 Belum ada subkategori.</p></div>';
            } else {
                ...
                html += paginationControls(
                    page,
                    subRes.total,
                    `window.renderSubcategoriesPage`
                );
            }

            window.renderSubcategoriesPage = function (p) {
                window.renderSubcategories(catId, p);
            };
            ...
            if (subList) {
                subList.innerHTML =
                    '<div class="placeholder"><p>❌ Gagal memuat subkategori.</p></div>';
            }
        }
    };
    ...
    window.showSubcategoryModal = function (
        ...
        const action = subId
            ? `window.updateSubcategory(${subId})`
            : `window.createSubcategory(${catId})`;
        const html = `
            <div class="modal-overlay show" id="modalOverlay">
                <div class="modal">
                    <div class="form-wrapper" style="overflow-y:auto; flex: 1;">
    ...
    window.createSubcategory = async function (catId) {
        ...
        if (!name || !price)
            return showToast("Semua kolom harus diisi.", "error");
        const formData = new FormData();
        formData.append("category_id", catId);
        formData.append("name", name);
        formData.append("price", price);

        console.log("[CRUD DEBUG] Creating subcategory", {
            catId,
            name,
            price
        });
    ...
    window.updateSubcategory = async function (subId) {
        ...
        try {
            const res = await fetch(`/api/products/subcategories/${subId}`, {
                method: "PUT",
                body: formData
            });
        ...
        window.showConfirmModal(
            ...
            async () => {
                const res = await fetch(
                    `/api/products/subcategories/${subId}`,
                    { method: "DELETE" }
                );
                const data = await res.json();
                showToast(data.message, data.success ? "success" : "error");
    ...
    window.renderItemSelector = async function (page = 1) {
        ...
        cats.data.forEach(c => {
            catSelectHtml += `<option value="${c.id}">${c.name}</option>`;
        });
        catSelectHtml += "</select>";

        let html = `
            <div class="form-group"
            onclick="window.showSearchableDropdown(document.getElementById('itemCatSelect'))">
            ...
            <div class="form-group"
            onclick="if(!document.getElementById('itemSubSelect').disabled)
            window.showSearchableDropdown(document.getElementById('itemSubSelect'))">
        ...
        document
            ...
            .addEventListener("change", async e => {
                ...
                const subRes = await fetch(
                    `/api/products/subcategories?category_id=${catId}&limit=100`
                );
    ...
    async function renderItems(subId, page) {
        const [itemRes, sumData] = await Promise.all([
            fetch(
                `/api/products/items?subcategory_id=${subId}&page=${page}&limit=${itemsPerPage}`
            ).then(r => r.json()),
        ...
        html +=
            '<button class="btn btn-primary" onclick="window.showItemModal(' +
            subId +
            ')">➕ Tambah</button>';
        html +=
            '<button class="btn btn-outline" onclick="window.importCSV(' +
            subId +
            ')">📥 Impor</button>';
        html +=
            '<button class="btn btn-outline" onclick="window.exportCSV(' +
            subId +
            ')">📤 Ekspor</button>';
        html += "</div>";
        const start = (page - 1) * itemsPerPage;
        ...
        html += paginationControls(
            page,
            itemRes.total,
            `window.renderItemsPage`
        );
        window.renderItemsPage = function (p) {
            renderItems(subId, p);
        };
    ...
    window.updateItem = async function (itemId) {
        ...
        const res = await fetch(`/api/products/items/${itemId}`, {
            method: "PUT",
            body: formData
        });
        const data = await res.json();
    ...
    window.deleteItem = function (itemId) {
        window.showConfirmModal("Yakin hapus item ini?", async () => {
            const res = await fetch(`/api/products/items/${itemId}`, {
                method: "DELETE"
            });
            const data = await res.json();
    ...
    window.exportCSV = async function (subId) {
        const res = await fetch(
            `/api/products/items/export?subcategory_id=${subId}`
        );
        const data = await res.json();
    ...
    window.importCSV = async function (subId) {
        ...
        fileInput.onchange = async e => {
            ...
            formData.append("subcategory_id", subId);
            const res = await fetch("/api/products/items/import/preview", {
                method: "POST",
                body: formData
            });
            const preview = await res.json();
            let html =
                '<div class="modal-overlay show"><div class="modal"><p>Pratinjau Impor</p>';
            preview.forEach(p => {
                html += `<div>${p.code} - ${p.status === "duplicate" ? "⚠️ Duplikat" : "✅ Baru"}</div>`;
            });
            html += `<div class="modal-actions">
                <button class="btn btn-primary" onclick="window.executeImport(${subId}, '${preview
    ...
    window.executeImport = async function (subId, codes) {
        ...
        formData.append("subcategory_id", subId);
        formData.append("codes", codes);
        const res = await fetch("/api/products/items/import/execute", {
            method: "POST",
            body: formData
        });
        const data = await res.json();
    ...
    window.loadDataTab = async function () {
        ...
        html += '<div class="btn-row">';
        html +=
            '<button class="btn btn-primary" onclick="window.exportAllData()">📤 Ekspor CSV Global</button>';
        html +=
            '<button class="btn btn-outline" onclick="window.importGlobalCSV()">📥 Impor CSV Global</button>';
        html += "</div>";
    ...
    function paginationControls(page, total, callback) {
        ...
        if (page > 1)
            html += `<button class="btn-page" onclick="(${callback})(${page - 1})">◀</button>`;
