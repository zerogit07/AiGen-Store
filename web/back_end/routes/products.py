...
from web.back_end.services.product_service import (
    ...
    get_subcategories, create_subcategory, update_subcategory, delete_subcategory,
    get_items, create_items, update_item, delete_item,
    export_items, import_csv_preview, import_csv_execute, get_product_stats
)
...
async def api_subcategories(category_id: int = Query(...), page: int = 1, limit: int = 10):
    offset = (page - 1) * limit
    return await get_subcategories(category_id, limit, offset)

@router.post("/subcategories")
async def api_create_subcategory(category_id: int = Form(...), name: str = Form(...), price: int = Form(...)):
    return await create_subcategory(category_id, name, price)

@router.put("/subcategories/{subcategory_id}")
async def api_update_subcategory(subcategory_id: int, name: str = Form(...), price: int = Form(...)):
...
async def api_import_preview(subcategory_id: int = Form(...), file: UploadFile = File(...)):
...
async def api_import_global(file: UploadFile = File(...)):
    import csv, io
    from source.database.queries import add_item, get_category_by_name, add_category, get_subcategory_by_name, add_subcategory
    ...
    for row in reader:
        if not row or len(row) < 4:
            continue
        cat_name, sub_name, code, harga = row[0].strip(), row[1].strip(), row[2].strip(), row[3].strip()
        ...
        added += 1
    return {"success": True, "message": f"Impor selesai. {added} item ditambahkan, {errors} error."}
